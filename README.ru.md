# Agentic Screencast — воспроизводимые ролики из сценария
...
Собирает видео из данных: вы описываете слайды и сцены, инструмент
синтезирует речь, считает кадры и склеивает готовый файл. Записи экрана нет —
каждый кадр это снимок веб-страницы в назначенный момент времени, поэтому
сборка воспроизводима, а сцены пересобираются по отдельности.
...
«ключ: значение», затем речь прозой. Проза первым классом, а не значением
в кавычках: её пишет человек и читает вслух синтез.
