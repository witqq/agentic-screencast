...
Агент изучает продукт, прототип, идею или исследование, пишет сценарий и
собирает видеопрезентацию для человека.
...
обзора функции, объяснения идеи и презентации с озвучкой. За факты, текст и проверку результата
отвечает агент; входную схему и отчёты сборки он получает в машинно читаемом виде.

Готовые страницы рендерятся в назначенные моменты, а живые действия
Playwright записываются в WebM вместе с курсором и откликом клика.
Неизменные сцены сборки берутся из кэша.
...
Начать можно со [скилла](skills/agentic-screencast/SKILL.md),
[готового примера](example/agent-video.md) и [точки входа агента](AGENTS.md).
...
Модуль `agentic-screencast/capture` предоставляет `recordTake` для новой
страницы Playwright и `capturePage` для уже открытой. Подготовка, включая вход,
идёт **до** начала записи. Методы `click`, `hover`, `type`, `press`, `drag` и
`range` принимают локаторы: Playwright ждёт доступности элемента и прокручивает
страницу, а инструмент ведёт видимый курсор. Импульс рисуется по настоящему
`pointerdown`, поэтому не опережает клик. `type` вводит символы по очереди и
отказывает для поля пароля. Время показа карточки вычисляется из её текста;
`{ until: locator }` ждёт результат действия без угаданной паузы.
`withFocusCard(locator, card, action)` ставит пояснение рядом с реальным
элементом на время действия: координаты карточки агенту вычислять не нужно.
...
[Запускаемый пример](example/live-capture.mjs) работает без внешнего сайта.
...
Общее поле `overlay` работает у `video`, `page` и других видов сцен.
Оно задаёт последовательность положений декоративного указателя, карточек
и наездов камеры на неподвижный кадр:

```markdown
## run · video
file: captures/real-run.mp4
overlay: {"pointer":[{"at":0.2,"x":0.2,"y":0.5},{"at":1.5,"x":0.7,"y":0.5,"click":true}],"cards":[{"at":0.4,"title":"Проверяем маршрут","body":"Курсор времени показывает каждый шаг.","position":"bottom-right"}]}
```

`at` — секунды от начала сцены; `x` и `y` — доли **готового кадра** от 0 до 1.
Курсор плавно перемещается между точками. `click:true` рисует волну, но не
нажимает кнопку в исходной записи. Для настоящих действий используйте
Playwright capture; ручной указатель на готовом видео — только графическая
аннотация. Карточка входит и выходит плавно; `position` принимает углы,
`center` или `near-focus`. Для набора текста укажите `reveal: "type"`, для
движения — `motion: "rise"`, `"pop"`, `"glide"` или `"fly"`; `enter` и `exit` задают
длительность входа/выхода. Карточка `fly` с полем `from` (`left`, `right`, `top`
или `bottom`) влетает из-за этого края кадра, чуть проскакивает, встаёт на место
и уходит тем же путём; без `fly` поле `from` — ошибка разбора. `hold` можно опустить: минимальное время чтения
вычисляется по длине текста и анимации. Между карточками нужно
не менее 0,35 секунды. Если видеоклип короче аннотаций, его последний кадр
удерживается до конца сцены. Подробнее — `agentic-screencast help overlay` и
[пример сценария](example/kinetic-video.md).

Для «объясняющей остановки» у видеосцены задайте `freezeAt` — секунду
реального клипа, кадр которой надо удержать. `overlay.camera` описывает
наезд: `at`, `move`, `hold`, `return`, `scale` и **одну** область `area`
в долях кадра `[left, top, width, height]`. У сохранённой страницы `page`
вместо `area` можно указать `target` — CSS-селектор элемента; камера
найдёт его геометрию сама. Между наездами остаётся 0,35 секунды. Подсветка
следует за областью, а карточка `near-focus` размещается рядом с ней.
`freezeAt` без камеры тоже создаёт неподвижную видеосцену; новый запуск
приложения для неё не нужен. [Рабочий пример](example/kinetic-video.md)
показывает вводную, клип и такой стоп-кадр в одном сценарии.
Для идеи или прототипа без живых кликов есть [самодостаточный пример](example/idea-video.md):
он показывает сохранённую HTML-страницу, наезд по CSS-селектору и надпись
«иллюстрация», чтобы зритель не принял макет за проверенный результат.

Камера у видеосцены работает и без `freezeAt`, поверх идущего клипа. Слой
тогда делится надвое: подсветка с затенением лежит под наездом и едет вместе
с картинкой, а карточки и подпись остаются прежнего размера поверх него.
Пока ведёт камера, нижняя подпись гаснет, чтобы на экране был один слой
текста. У сцены с `freezeAt` сборка меряет разброс яркости в каждой области
`area` на замороженном кадре и отказывает, если область почти пустая: в
отказе названы сцена и измеренный контраст.

### Замедление и остановка времени

Поле `speed` видеосцены переигрывает сам клип. Шаги идут по порядку
исходного клипа и не перекрываются:
...
Кусок `{from, to, rate}` проигрывает отрезок исходника с другой скоростью:
`0.5` — вдвое медленнее, `2` — вдвое быстрее, допустимо от 0,2 до 4.
Остановка `{at, hold}` удерживает секунду `at` исходника `hold` секунд готового
ролика (от 0,5 до 12) внутри того же плана, после чего клип идёт дальше.
Сцена удлиняется ровно на добавленное время, переигранный клип кэшируется.
Наезд и карточку ставьте на удержанные секунды, а не вырезайте стоп-кадр
в отдельную сцену: отдельная сцена даёт скачок и отрывает карточку от момента.
...
Базовую длительность определяет речь — сумма длин её тактов плюс хвост,
округлённая вверх до целого кадра. Аннотации могут продлить сцену до
окончания последней карточки или клика.
...
| `theme` | имя поставляемой темы или переменные оформления: уходят в корень страницы и в накладку | `midnight` |

Поставляемые темы — `midnight` (тёмная сине-серая, по умолчанию), `calm-paper`
(тёплая бумага, шрифт с засечками), `synthwave` (фиолетовая ночь, неон) и `noir`
(почти чёрная, один янтарный акцент). Тема оформляет слайды, подписи и карточки
разом. Запись `theme: {"preset":"noir","--acc":"#7aa2ff"}` берёт тему и меняет
одну переменную; объект без `preset` задаёт только свои переменные, как
прежде. Незнакомое имя — ошибка разбора со списком доступных тем. Список и
описание печатает `agentic-screencast help themes`. Слайды несут собственный
фоновый слой движения, появившиеся элементы слегка плывут, удержанные
карточки дышат; всё это функции времени сцены, поэтому кадры воспроизводимы.
...
Не решает за автора, какой сюжет, смысл действия, факт или голос показывать:
живые действия записываются, но проверить, что зритель их понял, должен агент.
